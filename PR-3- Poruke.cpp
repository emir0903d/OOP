...
public:
	int operator-(DatumVrijeme b) {
		long long int a = *_sati * 60 + *_minuti;
		long long int c = *b._sati * 60 + *b._minuti;
		return abs(a - c);
	}
	string ToString(stringFormat format_datuma) {

		if (format_datuma == stringFormat::DDMMGG) {
			return to_string(*_dan) +"."+ to_string(*_mjesec) + "." + to_string(*_godina%100);
		}
		if (format_datuma == stringFormat::DDMMGGGG) {
			return to_string(*_dan) + "." + to_string(*_mjesec) + "." + to_string(*_godina);
		}
		if (format_datuma == stringFormat::DDMMGGSSMM) {
			return to_string(*_dan) + "." + to_string(*_mjesec) + "." + to_string(*_godina%100)+ " " + to_string(*_sati) + ":" + to_string(*_minuti);
		}
		if (format_datuma == stringFormat::DDMMGGGGSSMM) {
			return to_string(*_dan) + "." + to_string(*_mjesec) + "." + to_string(*_godina) + " " + to_string(*_sati) + ":" + to_string(*_minuti);
		}
...
public:
	...
	Poruka(const Poruka& obj):_vrijeme(obj._vrijeme) {
		_posiljalac = obj._posiljalac;
		_primalac = obj._primalac;
		_sadrzaj = obj._sadrzaj;
	}
	Poruka& operator=(Poruka& obj) {
		_posiljalac = obj._posiljalac;
		_primalac = obj._primalac;
		_sadrzaj = obj._sadrzaj;
		_vrijeme = obj._vrijeme;
		return *this;
	}
...
public:
	...
	Komunikacija(const Komunikacija& obj) {
		_naslov = obj._naslov;
		_ocjena = obj._ocjena;
		_poruke = obj._poruke;
	}
	Komunikacija& operator=(Komunikacija &obj) {
		_naslov = obj._naslov;
		_ocjena = obj._ocjena;
		_poruke = obj._poruke;
		return *this;
...
class Korisnik {
	...
	char * _imePrezime;
	//char * - se odnosi na naziv predmeta ili tema gdje æe se èuvati kompletna komunikacija sa nastavnim osobljem ili 
	//korisnikom na odreðenom predmetu/temi
	...
	void OcijeniKomunikaciju(string naslov) {
		for (int i = 0; i < _komunikacija.getTrenutno(); i++) {
			if (string(_komunikacija.getElement1(i)) == naslov) {
				...
				if (float(prosjek) / float(br_poruka) <=5) {
					_komunikacija.getElement2(i).setOcjena(5);
				}
				if (float(prosjek) / float(br_poruka) > 5) {
					_komunikacija.getElement2(i).setOcjena(4);
				}
				if (float(prosjek) / float(br_poruka) > 10) {
					_komunikacija.getElement2(i).setOcjena(3);
				}
				if (float(prosjek) / float(br_poruka) >15) {
					_komunikacija.getElement2(i).setOcjena(2);
				}
				if (float(prosjek) / float(br_poruka) >20) {
					_komunikacija.getElement2(i).setOcjena(1);
				}
			}
		}
	}
	/*funkcija prikazuje sve poruke sa korisnikom cije korisnicko 
	ime je definisano parametrom, te vraca ukupan broj (razmijenjenih poruka sa tim korisnikom)*/
	int PrikaziKomunikacijuSaKorisnikom(string korisnik) {
		...
		for (int i = 0; i < _komunikacija.getTrenutno(); i++) {
			...
			for (int j = 0; j < _komunikacija.getElement2(i).getPoruke()->getTrenutno(); j++) {
				...
				if (_komunikacija.getElement2(i).getPoruke()->getElement1(j).getPosiljalac() == korisnik || _komunikacija.getElement2(i).getPoruke()->getElement1(j).getPrimalac() == korisnik) {
	...
	*/
	void PrikaziKomunikacijuPoNaslovu(string naslov, prikazPoruka prikaz) {
		...
		for (int i = 0; i < _komunikacija.getTrenutno(); i++) {
			if (string(_komunikacija.getElement1(i)) == naslov) {
				cout << "------------------------------------------------------" << endl;
				cout << _komunikacija.getElement2(i).getPoruke()->getElementi1Pok()->getDatumVrijeme().ToString(DDMMGGGGSSMM)<<endl;
				for (int j = 0; j < _komunikacija.getElement2(i).getPoruke()->getTrenutno(); j++) {
					...
					if (prikaz == prikazPoruka::SVE_PORUKE) {
						...
						else { cout << "PORUKA_IZBRISANA!"; }
					}
					if (prikaz == prikazPoruka::PRIKAZI_SAMO_POBRISANE) {
						if (_komunikacija.getElement2(i).getPoruke()->getElement2(j) == false) {
							cout << _komunikacija.getElement2(i).getPoruke()->getElement1(j).getSadrzaj()<<endl;
						}
					
					}
					if (prikaz == prikazPoruka::SAKRIJ_POBRISANE) {
						if (_komunikacija.getElement2(i).getPoruke()->getElement2(j) == true) {
							cout << _komunikacija.getElement2(i).getPoruke()->getElement1(j).getSadrzaj()<<endl;
						}
	...
	Korisnik & operator=(Korisnik& original) {
		int vel = strlen(original._imePrezime) + 1;
		_imePrezime = new char[vel];
		strcpy_s(_imePrezime, vel, original._imePrezime);
		_korisnickoIme = original._korisnickoIme;
...
/*
sve poruke se cuvaju u komunikaciji posiljaoca i primaoca tako da u svakom momentu oba ucesnika u komunikaciji posjeduju kompletnu
historiju poruka. poruke se grupisu na osnovu naslova, pa ce poruke pod naslovom "Sastanak" izmedju dva korisnika sadrzavati sve poruke
koje su razmijenjene na tu temu
u slucaju da posiljalac ili primalac ne postoje funkcija treba baciti izuzetak sa odgovarajucom porukom. takodjer, koristeci regex sve 
poruke koje sadrze neku od zabranjenih rijeci trebaju biti oznacene kao
*/
void PosaljiPoruku(Korisnik* niz[], int vel, string naslov, string posiljaoc, string primaoc, string sadrzaj, DatumVrijeme dat) {
	bool salje = false;
	bool prima = false;
	for (int i = 0; i < vel; i++) {
		if (string(niz[i]->getKorisnickoIme()) == posiljaoc) {
			salje = true;
		}
		if (string(niz[i]->getKorisnickoIme()) == primaoc) {
			prima = true;
		}
	}
	if (salje == false || prima == false) {
		throw exception("Nedovoljno ucesnika u komunikaciji!");
	}
	string pravilo = "(";
	...
	if (regex_search(sadrzaj, regex(pravilo))) {
		throw exception("Nedozvoljen sadrzaj!");
	}

	for (int i = 0; i < vel; i++) {
		if (niz[i]->getKorisnickoIme() == posiljaoc || niz[i]->getKorisnickoIme() == primaoc) {
			bool found = false;

			for (int j = 0; j < niz[i]->GetKomunikacija()->getTrenutno(); j++) {
				if (string(niz[i]->GetKomunikacija()->getElement1(j)) == naslov) {
					found = true;
					niz[i]->GetKomunikacija()->getElement2(j).getPoruke()->AddElement(Poruka(posiljaoc, primaoc, sadrzaj, dat), true);
				}

			}

			if(!found){
				Komunikacija temp01(naslov);
				temp01.getPoruke()->AddElement(Poruka(posiljaoc, primaoc, sadrzaj, dat), true);

				char * temp02 = new char[naslov.size() + 1];
				strcpy_s(temp02, naslov.size() + 1, naslov.c_str());
				niz[i]->GetKomunikacija()->AddElement(temp02, temp01);
			}
